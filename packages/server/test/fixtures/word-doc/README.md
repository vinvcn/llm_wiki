...
committed acceptance test `packages/server/test/legacy-doc.test.js` (run by
`npm test -w @llm-wiki/server`) to prove the web server's legacy `.doc`
extraction path (`packages/server/src/commands/preprocess.js` →
`word-extractor`, served through the `read_file` / `preprocess_file`
commands and the agent `source.search` tool) against genuine files — not
synthetic ones.
