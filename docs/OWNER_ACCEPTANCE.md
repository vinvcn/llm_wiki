...
1. **Every scenario carries three parts**: a scenario description (which flow,
...
2. **Include facts, observations, and reproducible details**: concrete data
...
3. **Be professional**: neutral, precise, blame-free wording; honest severity
   ratings; code defects stated separately from environmental factors.
4. **Link to the issue, PR, or commit whenever one exists.** A problem, fix, or
...
5. **Always state the context the test ran against**: commit under test, how
...
6. **Problems get follow-up references**: if a finding produced an issue or
   fix, the report carries the link both ways (the issue should also mention
   the session).
7. **Append-only history**: do not rewrite earlier reports; add dated
   correction notes.
8. **Clean results are recorded too** — a session that found nothing is
   evidence, not noise.
...
*Consolidated report: supersedes the seed version merged with PR #34; rewritten
to the strengthened rules (scenario description / testing done / verdict;
facts, observations, reproducible details) per owner instruction.*
